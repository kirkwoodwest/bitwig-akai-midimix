package com.kirkwoodwest.extensions.pluginwindow;
...
public class PluginWindowControlExtension extends ControllerExtension {
  ...
  public void init() {
    ...
    NumberSetting numberTracks = new NumberSetting(prefrences,"# Tracks for All", category, 1, 64, 1, "Tracks", NUM_TRACKS);
    NUM_TRACKS = numberTracks.get();

    NumberSetting numberSends = new NumberSetting(prefrences, "# Sends for All", category, 1, 16, 1, "Sends", NUM_SENDS);
    NUM_SENDS = numberSends.get();

    NumberSetting numberDevices = new NumberSetting(prefrences, "# Devices Per Channel", category, 1, 32, 1, "Devices", NUM_DEVICES);
    NUM_DEVICES = numberDevices.get();

    NumberSetting numberLayers = new NumberSetting(prefrences, "# Device Layers ", category, 1, 32, 1, "Device Layers", NUM_LAYERS);
    NUM_LAYERS = numberLayers.get();
